...
test('Merged group card - shows MERGED GROUP badge', async ({ page }) => {
...
test('Merged group card - Ad Volume active count is less than or equal to total', async ({ page }) => {
  await new KwiksAdsCreativeAgent(page).goto();
  const competitor = new Competitor(page);
  await competitor.navigate();
...
test('Merged group card - Format Split video + image equals total ad count', async ({ page }) => {
  await new KwiksAdsCreativeAgent(page).goto();
  const competitor = new Competitor(page);
  await competitor.navigate();
...
test('Merged group card - Ad Longevity testing + scaling + evergreen equals active count', async ({ page }) => {
  await new KwiksAdsCreativeAgent(page).goto();
  const competitor = new Competitor(page);
  await competitor.navigate();
